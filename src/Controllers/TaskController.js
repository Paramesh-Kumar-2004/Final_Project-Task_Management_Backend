...
export const removeSharedUser = async (req, res) => {
    try {
        console.log("Entered Into Remove Shared User")
        ...
        // Authorization: only task owner can remove shared users
        if (task.createdBy.toString() !== currentUserId.toString()) {
            return res.status(403).json({ message: "Not authorized to modify sharing" });
        }
        ...
        if (!isShared) {
            return res.status(400).json({ message: "User is not shared on this task" });
        }
